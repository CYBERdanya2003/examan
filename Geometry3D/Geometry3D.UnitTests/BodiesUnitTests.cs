...
namespace Geometry3DTests
{
    ...
    public class BodiesUnitTests
    {
        ...
        [TestCase(-6, -2, -4, 8, 6)]
        [TestCase(6, 2, -4, 7, 9)]
        [TestCase(6, -2, -4.5, 1.5, 3.5)]
        public void Cone_ContainsPoint_IsCorrect(double x, double y, double z, double radius, double sizeZ)
        {
            var cone = new Cone(new Vector3(x, y, z), radius, sizeZ);

            Assert.True(cone.ContainsPoint(new Vector3(x, y, z)));

            var angle = Math.Atan(radius / (sizeZ / 2));
            var pointX = x + radius * Math.Cos(angle);
            var pointY = y + radius * Math.Sin(angle);
            var pointZ = z + sizeZ / 2;
            Assert.True(cone.ContainsPoint(new Vector3(pointX, pointY, pointZ)));

            var pointX1 = x + radius / Math.Sqrt(8) * Math.Cos(angle);
            var pointY1 = y + radius / Math.Sqrt(8) * Math.Sin(angle);
            var pointZ1 = z + sizeZ / 4;
            Assert.True(cone.ContainsPoint(new Vector3(pointX1, pointY1, pointZ1)));

            Assert.False(cone.ContainsPoint(new Vector3(x + radius * Math.Sqrt(2), y + radius * Math.Sqrt(2), z + sizeZ)));
        }
        ...
        public void Cone_BoundingBox_IsCorrect(double x, double y, double z, double radius, double sizeZ)
        {
            ...
            var expectedBox = new RectangularCuboid(new Vector3(x, y, z), radius * 2, radius * 2, sizeZ);
            AssertCuboidsEqual(expectedBox, box);
        }
